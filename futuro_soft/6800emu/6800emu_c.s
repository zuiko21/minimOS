...
; usual overhead is 22+3=25 clock cycles
; boundary crossing
...
clear_z:		; +5
	...
	BRA next_op	; usual end

set_z:			; +5
	SMB2 ccr68	; set Z bit, *** Rockwell only! ***
	BRA next_op	; do not clear it

stab_nz:			; *store accumulator B and check NZ, +6 +6/10/14
	LDA b68			; store accumulator B
	BRA check_nz	; check N & Z
staa_nz:			; *store accumulator A and check NZ, +3 +6/10/14
	LDA a68			; store accumulator A
check_nz:			; *check N & Z, +6/10/14
	BNE cnz_nz		; if zero...
		SMB2 ccr68		; set Z
cnz_nz:
	BPL cnz_pl		; if negative...
		SMB3 ccr68		; set N
cnz_pl:
	BRA next_op		; most opcodes end this way

; valid opcode definitions
_01:
; NOP (2)
; no function, jump table will point to next_op, saving another 3 bytes
...
; +6, 6 bytes
	...
	BRA next_op	; standard end of routine (all +3 unless otherwise noted)
...
; +6, 6 bytes
	...
	BRA next_op	; standard end of routine

_08:
; INX (4)
; +13/13/22
	INC x68		; increase LSB
	BNE clear_z	; wrap is a rare case
		INC x68 + 1	; increase MSB
		BNE clear_z	; becoming zero is even rarer!
		BEQ set_z	; or BRA
...
; +
	LDA x68			; check LSB
	BNE dex_nw		; if zero, will wrap upon decrease!
		DEC x68 + 1		; wrap MSB
dex_nw:
	DEC x68			; otherwise just decrease LSB
	BNE clear_z		; if zero now, could be all zeroes!
		LDA x68 + 1		; check MSB
		BNE clear_z		; only if it went down to zero!
	BEQ set_z		; or BRA, rarest end
...
; +20/24/28
	...
	STA ccr68	; update status
	LDA a68		; get A
	JMP stab_nz	; store in B and check flags
...
; +20/24/28
	...
	STA ccr68	; update status
	LDA b68		; get B
	JMP staa_nz	; store in A and check flags
...
; +
	...
	SBC b68			; negate B
	TAX				; save!
	CMP #$80		; did change sign? maybe with native V bit?
...
; *** missing H flag ***
	...
	LDA ccr68		; get original flags
	BIT #%00000001	; mask for C flag
	BEQ adcam_do	; skip if C was clear
		SEC				; otherwise, set carry
adcam_do:
	AND #%11110000	; clear relevant bits
	STA ccr68		; update
	_PC_ADV			; not worth using the macro
	LDA (pc68), Y	; get operand
	ADC a68			; add accumulator A
...
; *** missing H flag ***
	...
	LDA ccr68		; get original flags
	BIT #%00000001	; mask for C flag
	BEQ adcad_do	; skip if C was clear
		SEC				; otherwise, set carry
adcad_do:
	AND #%11110000	; clear relevant bits
	STA ccr68		; update
	LDA a68			; get accumulator A
	ADC $4000, X	; add with carry
...
; *** missing H flag ***
	...
	LDA ccr68		; get original flags
	BIT #%00000001	; mask for C flag
	BEQ adcai_do	; skip if C was clear
		SEC				; otherwise, set carry
adcai_do:
	AND #%11110000	; clear relevant bits
	STA ccr68		; update
	LDA a68			; get accumulator A
	ADC (tmptr)		; add with carry
	STA a68			; update accumulator
	_CC_NZ			; check these
...
; *** missing H flag ***
	...
	LDA ccr68		; get original flags
	BIT #%00000001	; mask for C flag
	BEQ adcae_do	; skip if C was clear
		SEC				; otherwise, set carry
adcae_do:
	AND #%11110000	; clear relevant bits
	STA ccr68		; update
...
; *** missing H flag ***
	...
	LDA ccr68		; get original flags
	BIT #%00000001	; mask for C flag
	BEQ adcbm_do	; skip if C was clear
		SEC				; otherwise, set carry
adcbm_do:
	AND #%11110000	; clear relevant bits
	STA ccr68		; update
...
; *** missing H flag ***
	...
	LDA ccr68		; get original flags
	BIT #%00000001	; mask for C flag
	BEQ adcbd_do	; skip if C was clear
		SEC				; otherwise, set carry
adcbd_do:
	AND #%11110000	; clear relevant bits
	STA ccr68		; update
...
; *** missing H flag ***
	...
	LDA ccr68		; get original flags
	BIT #%00000001	; mask for C flag
	BEQ adcbi_do	; skip if C was clear
		SEC				; otherwise, set carry
adcbi_do:
	AND #%11110000	; clear relevant bits
	STA ccr68		; update
...
; *** missing H flag ***
	...
	LDA ccr68		; get original flags
	BIT #%00000001	; mask for C flag
	BEQ adcbe_do	; skip if C was clear
		SEC				; otherwise, set carry
adcbe_do:
	AND #%11110000	; clear relevant bits
	STA ccr68		; update
...
optable_l:
	.word	_00
	.word	next_op		; this is NOP, otherwise _01
	.word	_02
	.word	_03
	.word	_04
	.word	_05
