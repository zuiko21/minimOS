...
.(
	.asc	"Derivado de EhBASIC v2.22 por Lee Davison", 0		; comment with IMPORTANT attribution
	.asc	"RIP", 0
...
emptsk		= Ibuffe+1	; *** empty stack location *** $6F-70 [CHECK]
...
TK_END		= $80			; END token		; FIN
TK_FOR		= TK_END+1		; FOR token		; PARA
TK_NEXT		= TK_FOR+1		; NEXT token	; OTRO
TK_DATA		= TK_NEXT+1		; DATA token	; DATOS
TK_INPUT	= TK_DATA+1		; INPUT token	; PIDE
TK_DIM		= TK_INPUT+1	; DIM token		; CONJUNTO
TK_READ		= TK_DIM+1		; READ token	; TOMA
TK_LET		= TK_READ+1		; LET token		; SEA
TK_DEC		= TK_LET+1		; DEC token		; BAJA
TK_GOTO		= TK_DEC+1		; GOTO token	; VETE
TK_RUN		= TK_GOTO+1		; RUN token		; VAMOS
TK_IF		= TK_RUN+1		; IF token		; SI
TK_RESTORE	= TK_IF+1		; RESTORE token	; RECUPERA
TK_GOSUB	= TK_RESTORE+1	; GOSUB token	; VISITA
TK_RETIRQ	= TK_GOSUB+1	; RETIRQ token
TK_RETNMI	= TK_RETIRQ+1	; RETNMI token
TK_RETURN	= TK_RETNMI+1	; RETURN token	; VUELVE
TK_REM		= TK_RETURN+1	; REM token		; ES
TK_STOP		= TK_REM+1		; STOP token	; TERMINA
TK_ON		= TK_STOP+1		; ON token		; SEGUN
TK_NULL		= TK_ON+1		; NULL token	; NULO
TK_INC		= TK_NULL+1		; INC token		; SUBE
TK_WAIT		= TK_INC+1		; WAIT token	; ESPERA
TK_LOAD		= TK_WAIT+1		; LOAD token	; CARGA
TK_SAVE		= TK_LOAD+1		; SAVE token	; GUARDA
TK_DEF		= TK_SAVE+1		; DEF token
TK_POKE		= TK_DEF+1		; POKE token	; METE
TK_DOKE		= TK_POKE+1		; DOKE token	; METEDOS
TK_CALL		= TK_DOKE+1		; CALL token	; LLAMA
TK_DO		= TK_CALL+1		; DO token		; HAZ
TK_LOOP		= TK_DO+1		; LOOP token	; REPITE
TK_PRINT	= TK_LOOP+1		; PRINT token	; ESCRIBE
TK_CONT		= TK_PRINT+1	; CONT token	; SIGUE
TK_LIST		= TK_CONT+1		; LIST token	; LISTA
TK_CLEAR	= TK_LIST+1		; CLEAR token	; BORRA
TK_NEW		= TK_CLEAR+1	; NEW token		; NUEVO
TK_WIDTH	= TK_NEW+1		; WIDTH token	; ANCHO
TK_GET		= TK_WIDTH+1	; GET token		; TECLA
TK_SWAP		= TK_GET+1		; SWAP token	; CAMBIA
TK_BITSET	= TK_SWAP+1		; BITSET token	; PONBIT
TK_BITCLR	= TK_BITSET+1	; BITCLR token	; QUITABIT
...
TK_TAB		= TK_BYE+1		; TAB token		; TABULADOR ##### SYS no longer used, minimOS needs BYE #####
TK_ELSE		= TK_TAB+1		; ELSE token	; CONTRARIO
TK_TO		= TK_ELSE+1		; TO token		; HASTA
TK_FN		= TK_TO+1		; FN token
TK_SPC		= TK_FN+1		; SPC token		; ESPACIOS
TK_THEN		= TK_SPC+1		; THEN token	; ENTONCES
TK_NOT		= TK_THEN+1		; NOT token		; NO
TK_STEP		= TK_NOT+1		; STEP token	; PASO
TK_UNTIL	= TK_STEP+1		; UNTIL token	; ACABANDO
TK_WHILE	= TK_UNTIL+1	; WHILE token	; MIENTRAS
TK_OFF		= TK_WHILE+1	; OFF token		; APAGA
...
TK_AND		= TK_POWER+1	; AND token		; MAS
TK_EOR		= TK_AND+1		; EOR token		; DISTINTO
TK_OR		= TK_EOR+1		; OR token		; OBIEN
TK_RSHIFT	= TK_OR+1		; RSHIFT token	; MITAD
TK_LSHIFT	= TK_RSHIFT+1	; LSHIFT token	; DOBLE
...
TK_SGN		= TK_LT+1		; SGN token		; SIGNO
TK_INT		= TK_SGN+1		; INT token		; ENTERO
TK_ABS		= TK_INT+1		; ABS token		; ABSOLUTO
TK_USR		= TK_ABS+1		; USR token		; USUARIO
TK_FRE		= TK_USR+1		; FRE token		; LIBRE
TK_POS		= TK_FRE+1		; POS token		; POSICION
TK_SQR		= TK_POS+1		; SQR token		; RAIZ
TK_RND		= TK_SQR+1		; RND token		; AZAR
TK_LOG		= TK_RND+1		; LOG token		; LOGARITMO
TK_EXP		= TK_LOG+1		; EXP token		; EXPONENCIAL
TK_COS		= TK_EXP+1		; COS token		; COSENO
TK_SIN		= TK_COS+1		; SIN token		; SENO
TK_TAN		= TK_SIN+1		; TAN token		; TANGENTE
TK_ATN		= TK_TAN+1		; ATN token		; ARCOTANGENTE
TK_PEEK		= TK_ATN+1		; PEEK token	; SACA
TK_DEEK		= TK_PEEK+1		; DEEK token	; SACADOS
TK_SADD		= TK_DEEK+1		; SADD token	; LUGAR
TK_LEN		= TK_SADD+1		; LEN token		; LARGO
TK_STRS		= TK_LEN+1		; STR$ token	; TEXTO$
TK_VAL		= TK_STRS+1		; VAL token		; VALOR
TK_ASC		= TK_VAL+1		; ASC token		; CODIGO
TK_UCASES	= TK_ASC+1		; UCASE$ token	; MAYUSCULA$
TK_LCASES	= TK_UCASES+1	; LCASE$ token	; MINUSCULA$
TK_CHRS		= TK_LCASES+1	; CHR$ token	; LETRA$
TK_HEXS		= TK_CHRS+1		; HEX$ token	; HEXADECIMAL$
TK_BINS		= TK_HEXS+1		; BIN$ token	; BINARIO$
TK_BITTST	= TK_BINS+1		; BITTST token	; MIRABIT
TK_MAX		= TK_BITTST+1	; MAX token		; MAXIMO
TK_MIN		= TK_MAX+1		; MIN token		; MINIMO
TK_PI		= TK_MIN+1		; PI token
TK_TWOPI	= TK_PI+1		; TWOPI token	; DOSPI
TK_VPTR		= TK_TWOPI+1	; VARPTR token	; DIRECCION
TK_LEFTS	= TK_VPTR+1		; LEFT$ token	; IZQUIERDA$
TK_RIGHTS	= TK_LEFTS+1	; RIGHT$ token	; DERECHA$
TK_MIDS		= TK_RIGHTS+1	; MID$ token	; MEDIO$
...
; *** will arrive here if some amount was specified ***
LAB_2DAA
...
;	BCC	LAB_13EA		; if < keyword first character table byte go restore
	BCC	PATCH_LC2		; if < keyword first character table byte go restore *** mixed case patch
...
;	CMP	Ibuffs,X		; compare with byte from input buffer
	EOR	Ibuffs,X		; check bits against table *** patch
	AND	#$DF			; mask lower case bit *** patch
...
LAB_13EC
	...
	SBC	#$3A			; subtract $3A (carry set by CMP #00)
	BEQ	LAB_13FF		; branch if it was $3A (is now $00)

						; A now holds token-$3A
	CMP	#TK_DATA-$3A	; compare with DATA token - $3A
	BNE	LAB_1401		; branch if not DATA

						; token was $3A or DATA
LAB_13FF
	STA	Oquote			; save token-$3A (clear for $3A, TK_DATA-$3A for DATA)
LAB_1401
...
LAB_FOR
	...
	JSR	LAB_SNBS		; scan for next BASIC statement ([$3A] or [EOL])
	CLC					; clear carry for add
	TYA					; copy index to A
...
; this is the CMOS emulation of absolute indexed indirect jump
;	TAY					; copy to index
...
; improved CMOS version
	TAX					; copy to (different) index
	JSR LAB_IGBY		; jump to increment and scan memory, and return
	JMP (LAB_CTBLC, X)	; then jump to vector, note different list without the -1 offset
...
LAB_CONT
	...
	LDX	#$1E			; error code $1E ("cannot continue" error)
...
LAB_LOOP
	...
	CMP	#$3A			; could be $3A
	BEQ	LoopAlways		; if [$3A]... loop forever
...
LoopDone
	...
	TXS					; correct stack *** OK for 65816
	JMP	LAB_DATA		; go perform DATA (find $3A or [EOL])
...
; scan for next BASIC statement ([$3A] or [EOL])
; returns Y as index to [$3A] or [EOL]

LAB_SNBS
	LDX	#$3A			; set look for character = $3A
...
; there was no ELSE so continue execution of IF <expr> THEN <stat> [[$3A] <stat>]. any
...
LAB_1773
	...
	JSR	LAB_GFPN		; get fixed-point number into temp integer (skip this n)
						...
						; no we cannot, what if we meet a $3A or [EOL]?)
...
LAB_1988
	...
	LDA	#$3A			; else search character is $3A
...
LAB_19DD
	JSR	LAB_SNBS		; scan for next BASIC statement ([$3A] or [EOL])
	INY					; increment index
	TAX					; copy character ([$3A] or [EOL])
	BNE	LAB_19F6		; branch if [$3A]
...
; for functions that returned strings *** may be improved for CMOS
...
LAB_1EA1
	...
	JSR	LAB_121F		; check available memory, "Out of memory" error if no room
						; addr to check is in AY (low/high)
	LDY	#$00			; clear Y (do not need to clear A)
...
LAB_2216
	...
	JSR	LAB_11D6		; open up space in memory, do not set array end
...
LAB_229C
	TAX					; copy length to index (do not count with Y)
...
LAB_2A74
	...
	ADC	#$3A			; add character $3A ($30+$0A, result is 10 less that value)
...
; automatically enable the interrupt when we exit
	...
	JMP	LAB_15C2		; go do interpreter inner loop
						; cannot RTS, we used the stack! the RTS from the ctrl-c
...
; For a 1.536MHz 65C02 including the JSR and RTS
; fastest (>=$3A)	=  29 cycles =  18.9uS
; slowest (<$3A)	=  40 cycles =  26uS
; space skip	= +21 cycles = +13.7uS
; inc across page	=  +4 cycles =  +2.6uS
...
LAB_2CF4
	...
	CMP	#$3A			; compare with $3A
	BCS	LAB_2D05		; exit if >= $3A, not numeric, carry set
...
LAB_CTBLC
	...
	.word	LAB_NEXT		; NEXT
	.word	LAB_DATA		; DATA
	.word	LAB_INPUT		; INPUT
	.word	LAB_DIM			; DIM
	.word	LAB_READ		; READ
	.word	LAB_LET			; LET
	.word	LAB_DEC			; DEC		new command
	.word	LAB_GOTO		; GOTO
	.word	LAB_RUN			; RUN
	.word	LAB_IF			; IF
	.word	LAB_RESTORE		; RESTORE	modified command
	.word	LAB_GOSUB		; GOSUB
	.word	LAB_RETIRQ		; RETIRQ	new command
	.word	LAB_RETNMI		; RETNMI	new command
	.word	LAB_RETURN		; RETURN
	.word	LAB_REM			; REM
	.word	LAB_STOP		; STOP
	.word	LAB_ON			; ON		modified command
	.word	LAB_NULL		; NULL		modified command
	.word	LAB_INC			; INC		new command
	.word	LAB_WAIT		; WAIT
	.word	V_LOAD			; LOAD
	.word	V_SAVE			; SAVE
	.word	LAB_DEF			; DEF
	.word	LAB_POKE		; POKE
	.word	LAB_DOKE		; DOKE		new command
	.word	LAB_CALL		; CALL		new command
	.word	LAB_DO			; DO		new command
	.word	LAB_LOOP		; LOOP		new command
	.word	LAB_PRINT		; PRINT
	.word	LAB_CONT		; CONT
	.word	LAB_LIST		; LIST
	.word	LAB_CLEAR		; CLEAR
	.word	LAB_NEW			; NEW
	.word	LAB_WDTH		; WIDTH		new command
	.word	LAB_GET			; GET		new command
	.word	LAB_SWAP		; SWAP		new command
	.word	LAB_BITSET		; BITSET	new command
...
TAB_CHRT
	...
	.word	TAB_ASCQ		; table for "Q", Spanish only
...
;	.word	TAB_ASCW		; table for "W", not in Spanish
...
TAB_ASCA	; ** A **
LBB_ABS
	.byte	"BSOLUTO(",TK_ABS	; ABS(		; ABSOLUTO
LBB_UNTIL
	.byte	"CABANDO",TK_UNTIL	; UNTIL		; ACABANDO
LBB_WIDTH
	.byte	"NCHO",TK_WIDTH		; WIDTH		; ANCHO
LBB_OFF
	.byte	"PAGA",TK_OFF		; OFF		; APAGA
LBB_ATN
	.byte	"RCOTANGENTE(",TK_ATN	; ATN(	; ARCOTANGENTE
LBB_RND
	.byte	"ZAR(",TK_RND		; RND(		; AZAR
	.byte	$00
TAB_ASCB	; ** B **
LBB_DEC
	.byte	"AJA",TK_DEC		; DEC		; BAJA
LBB_BINS
	.byte	"INARIO$(",TK_BINS	; BIN$(		; BINARIO$
LBB_CLEAR
	.byte	"ORRA",TK_CLEAR		; CLEAR		; BORRA
LBB_BYE
	.byte	"YE",TK_BYE			; BYE		##### for minimOS #####
	.byte	$00
TAB_ASCC	; ** C **
LBB_SWAP
	.byte	"AMBIA",TK_SWAP		; SWAP		; CAMBIA
LBB_LOAD
	.byte	"ARGA",TK_LOAD		; LOAD		; CARGA
LBB_ASC
...
TAB_ASCD	; ** D **
LBB_DATA
	.byte	"ATOS",TK_DATA		; DATA		; DATOS
LBB_DEF
	.byte	"EF",TK_DEF			; DEF
LBB_RIGHTS
	.byte	"ERECHA$(",TK_RIGHTS; RIGHT$(	; DERECHA$
LBB_VPTR
	.byte	"IRECCION(",TK_VPTR	; VARPTR(	; DIRECCION
LBB_EOR
	.byte	"ISTINTO",TK_EOR	; EOR		; DISTINTO
LBB_TWOPI
	.byte	"OSPI",TK_TWOPI		; TWOPI		; DOSPI
	.byte	$00
TAB_ASCE	; ** E **
LBB_INT
	.byte	"NTERO(",TK_INT		; INT(		; ENTERO
LBB_THEN
...
TAB_ASCG	; ** G **
LBB_SAVE
	.byte	"UARDA",TK_SAVE		; SAVE		; GUARDA
	.byte	$00
TAB_ASCH	; ** H **
LBB_TO
	.byte	"ASTA",TK_TO		; TO		; HASTA
LBB_DO
	.byte	"AZ",TK_DO			; DO		; HAZ
LBB_HEXS
...
LBB_IRQ
	.byte	"RQ",TK_IRQ			; IRQ
LBB_LEFTS
	.byte	"ZQUIERDA$(",TK_LEFTS	; LEFT$(; IZQUIERDA$
	.byte	$00
TAB_ASCL	; ** L **
LBB_LEN
	.byte	"ARGO(",TK_LEN		; LEN(		; LARGO
LBB_CHRS
	.byte	"ETRA$(",TK_CHRS	; CHR$(		; LETRA$
LBB_FRE
	.byte	"IBRE(",TK_FRE		; FRE(		; LIBRE
LBB_LIST
	.byte	"ISTA",TK_LIST		; LIST		; LISTA
LBB_CALL
	.byte	"LAMA",TK_CALL		; CALL		; LLAMA
LBB_LOG
	.byte	"OGARITMO(",TK_LOG	; LOG(		; LOGARITMO
LBB_SADD
...
LBB_UCASES
	.byte	"AYUSCULA$(",TK_UCASES; UCASE$(	; MAYUSCULA$
LBB_MIDS
	.byte	"EDIO$(",TK_MIDS	; MID$(		; MEDIO$
LBB_DOKE
	.byte	"ETEDOS",TK_DOKE	; DOKE		; METEDOS ** before METE **
LBB_POKE
	.byte	"ETE",TK_POKE		; POKE		; METE
LBB_WHILE
	.byte	"IENTRAS",TK_WHILE	; WHILE		; MIENTRAS
LBB_MIN
	.byte	"INIMO(",TK_MIN		; MIN(		; MINIMO
LBB_LCASES
	.byte	"INUSCULA$(",TK_LCASES; LCASE$(	; MINUSCULA$
LBB_BITTST
	.byte	"IRABIT(",TK_BITTST	; BITTST(	; MIRABIT
	.byte	$00
TAB_ASCN	; ** N **
LBB_NMI
	.byte	"MI",TK_NMI			; NMI
...
LBB_INPUT
	.byte	"IDE",TK_INPUT		; INPUT		; PIDE ** before PI **
LBB_PI
	.byte	"I",TK_PI			; PI
LBB_BITSET
	.byte	"ONBIT",TK_BITSET	; BITSET	; PONBIT
LBB_POS
	.byte	"OSICION(",TK_POS	; POS(		; POSICION
	.byte	$00
TAB_ASCQ	; ** Q, Spanish only **
LBB_BITCLR
	.byte	"UITABIT",TK_BITCLR	; BITCLR	; QUITABIT
	.byte	$00
TAB_ASCR	; ** R **
LBB_SQR
	.byte	"AIZ(",TK_SQR		; SQR(		; RAIZ
LBB_RESTORE
	.byte	"ECUPERA",TK_RESTORE; RESTORE	; RECUPERA
LBB_LOOP
	.byte	"EPITE",TK_LOOP		; LOOP		; REPITE
LBB_RETIRQ
...
TAB_ASCS	; ** S **
LBB_DEEK
	.byte	"ACADOS(",TK_DEEK	; DEEK(		; SACADOS ** before SACA **
LBB_PEEK
	.byte	"ACA(",TK_PEEK		; PEEK(		; SACA
LBB_LET
	.byte	"EA",TK_LET			; LET		; SEA
LBB_ON
	.byte	"EGUN",TK_ON		; ON		; SEGUN
LBB_SIN
	.byte	"ENO(",TK_SIN		; SIN(		; SENO
LBB_SGN
	.byte	"IGNO(",TK_SGN		; SGN(		; SIGNO ** before SI **
LBB_CONT
	.byte	"IGUE",TK_CONT		; CONT		; SIGUE ** before SI **
LBB_IF
	.byte	"I",TK_IF			; IF		; SI
LBB_INC
	.byte	"UBE",TK_INC		; INC		; SUBE
	.byte	$00
TAB_ASCT	; ** T **
LBB_TAB
...
LBB_GET
	.byte	"ECLA",TK_GET		; GET		; TECLA
LBB_STOP
	.byte	"ERMINA",TK_STOP	; STOP		; TERMINA
LBB_STRS
	.byte	"EXTO$(",TK_STRS	; STR$(		; TEXTO$
LBB_READ
	.byte	"OMA",TK_READ		; READ		; TOMA
	.byte	$00
TAB_ASCU	; ** U **
LBB_USR
	.byte	"SUARIO(",TK_USR	; USR(		; USUARIO
	.byte	$00
TAB_ASCV	; ** V **
LBB_VAL
	.byte	"ALOR(",TK_VAL		; VAL(		; VALOR
LBB_RUN
	.byte	"AMOS",TK_RUN		; RUN		; VAMOS
LBB_GOTO
	.byte	"ETE",TK_GOTO		; GOTO		; VETE
LBB_GOSUB
	.byte	"ISITA",TK_GOSUB	; GOSUB		; VISITA
LBB_RETURN
	.byte	"UELVE",TK_RETURN	; RETURN	; VUELVE
	.byte	$00
;TAB_ASCW
;	.byte	$00
TAB_POWR
