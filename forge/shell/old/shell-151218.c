...
// right, continue checking line
		...
		if (x==128) {					// valid opcode found
				...
				i = 0;						// now printed chars!
				printf("_%04x: ", ptr);		// address as label for most assemblers
				do {
					c = ram[++oldscan+rom] & 127;	// get char from opcode list
					switch(c) {
						case '%':				// relative addressing
				...
				} while (ram[rom+oldscan]<128);		// until terminator is found
				while (i<16) {				// tabulate!
					printf(" ");
					i++;
				}
				printf("; ");						// get ready for hex dump
		} else {
			count = 0;					// stay here
			printf("***ERROR***\n");	// invalid string
		}
// should "poke" tmp[2], tmp[1], tmp[0] at ptr...ptr+2
		for(i=0;i<count;i++) {
			printf("%02x ", tmp[2-i]);	// value to be poked
