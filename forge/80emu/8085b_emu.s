...
; H is half carry (for BCD, not testable), reset by OR/XOR *** NOT USED HERE
; P is parity, sets on logical/rots (and more?) when number of 1s is EVEN *** NOT USED HERE
...
; *** interrupt support ***
; unsupported opcodes will be TRAPped *** including use of H or P

_08:_10:_18:_28:_38:_d9:
_cb:_ed:_dd:_fd:
_27:_ea:_e2:_ec:_e4:_e8:_e0:
...
iflags:
	LDA f80		; get previous status (+19/21/23)
	AND #%00111111	; reset SZ
...
dflags:
	...
	JMP next_op	; standard end, no parity nor H!
...
; ANA M (7)
;+49/51.5/54
; ***** Logic ops clear C (at least ANA/ANI) ***** no HP
	_MEMORY		; prepare pointer
	LDA (tmptr)	; variable term
anai:
	TAX		; keep data here (+27/29/31)
anam:
	LDA f80		; get old flags (+25/27/29)
	AND #%00111110	; reset S, Z & C
	STA f80		; store base flags
	TXA		; retrieve value
	AND a80		; logical AND
	STA a80		; store result
	_CC_SZ		; check sign & zero bits
	JMP next_op	; no parity!

_a7:_b7:
; ORA A (4) same as ANA A as no H bit here!
...
;+55/59.5/64 no HP
	...
	LDA (tmptr)	; variable term
cmpi:
	TAX		; will receive value here (+33/37/41)
cmpm:
	STX tmptr	; keep first operand! (+31/35/39)
	LDA f80		; get old flags
	AND #%00111110	; reset S, Z & C
	STA f80		; store base flags
...
addi:
	TAX		; eeeeek (+32/36/40)
addm:
	LDA f80		; old flags (+30/34/38)
	AND #%00111110	; clear SZC
	STA f80		; store base flags
	TXA		; get operand
...
adci:
	TAX		; eeeeek (+35/37/39)
adcm:
	LDA f80		; get old flags (+33/35/37)
	LSR		; copy emulated C on native carry! 
; ** much better since both native and emulated C match! **
	AND #%00011111	; clear SZ, note shift
	STA f80		; store base flags
	TXA		; get operand
	ADC a80	; addition with carry
	STA a80		; store result
	_CC_SZ		; check sign & zero bits
...
subi:
	TAX				; eeeeek (+36/40/44)
subm:
	STX tmptr		; keep first operand! (+34/38/42)
	LDA f80			; old flags
	AND #%00111110	; clear SZC
	STA f80			; store base flags
...
;		CLC		; or clear native C (sets borrow)
sbb_c:
	AND #%00111110	; clear SZC
	STA f80			; store base flags
	LDA a80			; look at accumulator
