...
;#include "drivers/drv_led.h"	; disabled for debugging
...
k_vec:			; kernel's entry points
	...
	.word hid_push	; insert into input queue (deprecated -- will be 'seconds' in 0.4.1)
...
drv_led:			; should be default device, currently HERE for debug, put after dummy
;#include "drivers/drv_led.s"	; disabled for debugging
...
;DEBUG CODE
lda #$ff		; all out for Porculete
sta _VIA+_ddra
sta _VIA+_ddrb
lda #$eb		; right display
sta _VIA+_iora
lda #$be		; left display
sta _VIA+_iorb

/*
;debug code, disabled section
bra rtest
asc	"RAMtest" ; should NOT appear!
; *** SRAM test ***
rtest:
...
;	JSR init_led		; preliminary reset, should be from table, disabled for debugging
;	JSR init_ss22
;	JSR init_acia
;	JSR init_lcd
...
; *** final startup! ***
	...
	LDA #>isr
	STA irqvec+1
;	CLI			; enable interrupts DEBUG
; say hello!
	LDA #<hello		; LSB of the string
	STA z10
	LDA #>hello		; MSB
	STA z10+1
	LDY #led_dev		; default device, currently LED
;	_KERNEL(_string)	; print the message, disabled for debugging
; do something useful...
;	JMP shell		; launch monitor/shell

;DEBUG CODE
lda #$fe		; right display
sta _VIA+_iora
lda #$ca		; left display
sta _VIA+_iorb
debug_lock:	bra debug_lock
...
;	BEQ deb_wait	; NUL at end of string DEBUG
	STA z2		; put parameter
;	_KERNEL(_cout)	; character output, shouldn't use regular kernel, but... DEBUG
...
inc _VIA+_iorb
debug_nocarry:
bra deb_wait	; forever

; DEBUG won't reach here
; ***** read key and wait until OK is pressed
;	JSR led_cin	; read keypad buffer DEBUG
...
;	JSR led_poll		; very important, muxes display and keypad DEBUG
isr_done:
	_PLY	; restore registers
	_PLX
	PLA
...
; help for manual linking...
	.asc	" minimOS 0.4b3 DEBUG 1c @ 20130505 ", 0
