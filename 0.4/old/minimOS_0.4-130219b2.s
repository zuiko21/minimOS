...
page_ok:
	...
	BNE ramtest
	LDX #6			; special test for zeropage, respect test results!
	LDA #$AA		; test pattern
zerotest:
	STA 0, X		; store it
	CMP 0, X		; reads OK?
...
measure:
	...
	STA user+1		; safe (this far) zeropage vector
	_LDAX(user)		; last non-mirrored page number, anywhere in the page
...
; *** set default I/O device ***
	_STZA default_out	; should check some devices, now defaults to LED keypad
	_STZA default_in
...
shell:				; so far, a post-POST task only!
	LDY #0			; default device, should be LED keypad
